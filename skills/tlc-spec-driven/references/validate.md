...
**Report contract (external collectors parse this file literally):**

- **Diff range** uses the exact shape `` **Diff range**: `<base>..<tip>` (N commits) `` where `<base>` = `git merge-base main HEAD`, `<tip>` = the branch HEAD, and `N` = `git rev-list --count <base>..<tip>`. Run the commands; a range or count written from memory is measured as a divergent declaration.
- **An AC ID may only appear on a line that carries its live runtime evidence** — a `tests/<file>:<line>` citation that resolves in the tree (in this project the runtime tree is `tests/`; unit-file citations under `src/` prove structure, not execution). Every line that names an AC without such a citation is measured as a runtime-unverified AC. One line per AC; do not repeat IDs in prose.
- The verdict must read PASS before the feature merges — a FAIL verdict stays attached to the unit in the sample.
...
| Requirement | Previous Status | New Status   | Runtime evidence          |
| ----------- | --------------- | ------------ | ------------------------- |
| [FEAT]-01   | Implementing    | ✅ Verified  | `tests/test_feat.py:[NN]` |
| [FEAT]-02   | Implementing    | ❌ Needs Fix | `tests/test_feat.py:[NN]` |
