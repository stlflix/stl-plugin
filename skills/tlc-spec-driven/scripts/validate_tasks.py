...
What it checks (heuristic markdown inspection, not a full parser):
  ...
  ERROR  - the sibling spec.md declares a requirement ID that tasks.md never
           names (an orphan AC - the same rule the score's `ac_orphan` sensor
           measures after the merge, when it is too late to fix)
  ...
  WARN   - no sibling spec.md next to tasks.md (orphan check skipped)
...
# Same shape the score collector's `_REQUIREMENT_ID` matches - the two gates
# must agree on what a requirement ID is, or a tasks.md that passes here can
# still be measured as carrying orphans after the merge.
REQUIREMENT_ID_RE = re.compile(r"\b([A-Z][A-Z0-9]*)-(\d+)\b")


def requirement_ids(text):
    """The requirement IDs of the dominant prefix in a spec, in declaration order.

    A spec cites AD-012 or JIRA-47 alongside its own FEAT-nn; the prefix that
    appears most is the one the document declares, and the rest are links.
    Mirrors the score collector's `_requirement_ids` so the pre-approval gate
    and the post-merge `ac_orphan` sensor answer the same question.
    """
    found = REQUIREMENT_ID_RE.findall(text)
    if not found:
        return []
    counts = {}
    for prefix, _number in found:
        counts[prefix] = counts.get(prefix, 0) + 1
    dominant = max(counts.items(), key=lambda pair: (pair[1], pair[0]))[0]
    seen = []
    for prefix, number in found:
        ident = "%s-%s" % (prefix, number)
        if prefix == dominant and ident not in seen:
            seen.append(ident)
    return seen


def orphan_ids(spec_text, tasks_text):
    """Requirement IDs the spec declares and tasks.md never names (substring
    membership, exactly as the collector's `_measure_orphans` counts them)."""
    return [ident for ident in requirement_ids(spec_text) if ident not in tasks_text]
...
def check(tasks_path):
    ...
    # Orphan ACs against the sibling spec.md - the score's `ac_orphan` sensor,
    # asked while the tasks can still be fixed instead of after the merge.
    spec_path = os.path.join(os.path.dirname(os.path.abspath(tasks_path)), "spec.md")
    if not os.path.isfile(spec_path):
        warnings.append("no sibling spec.md next to tasks.md - orphan check skipped")
    else:
        with open(spec_path, "r", encoding="utf-8") as f:
            spec_text = f.read()
        tasks_text = "\n".join(lines)
        for ident in orphan_ids(spec_text, tasks_text):
            errors.append(f"{ident} is declared in spec.md and named by no task (ac_orphan)")
