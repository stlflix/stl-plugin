...
6. **Spec freeze:** approving tasks freezes the EARS text of `spec.md` and `design.md`. A requirement that must change after that point reopens Tasks — amend the spec, re-run the validation scripts, re-present for approval — never a silent in-place edit during Execute. (Post-approval churn is measured per commit from git; ticking checkboxes and updating traceability are not churn.)
7. **Repair in place, never on top:** when a task's gate fails after its commit, fix by amending or squashing into that task's own commit before the merge. A `fix:`/`revert:` commit inside the feature branch is measured as intra-range rework; the merge should land exactly one commit per task.
