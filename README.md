...
| MCP `buildloop` | `.mcp.json` | your own database on the ops BuildLoop cluster (`db.stlflix.com.br/mcp`); bearer token via `userConfig` |
...
| `mcp-server` | `mcp-server/` | the BuildLoop MCP server behind the ops cluster (deployed, not shipped to collaborators) |
| templates | `templates/` | `CLAUDE.global.md`, `CLAUDE.project.md`, `gitignore.default`, `handoff.md` |

The ops database cluster is reached through **our own** MCP server (`mcp-server/`),
not a vendor MCP. Each collaborator gets one database and
